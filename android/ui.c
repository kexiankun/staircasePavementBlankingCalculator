...
/*渲染GUI*/
static void *
render_gui(struct nk_context *ctx, SDL_Window *win, int w, int h, int dpi) {
  int RH = h / 10;

  if (nk_begin(ctx, "    楼梯铺装下料算器1.0.1.20220503_beta", nk_rect(0, 0, w, h),
          NK_WINDOW_CLOSABLE | 
...
popup_active = 1;
    ...
    if (event33 & NK_EDIT_ACTIVATED) { SDL_StartTextInput(); }

    if (event33 & NK_EDIT_DEACTIVATED) { SDL_StopTextInput(); }
    ...
    if (event4 & NK_EDIT_ACTIVATED) { SDL_StartTextInput(); }

    if (event4 & NK_EDIT_DEACTIVATED) { SDL_StopTextInput(); }
    //平台数据
    ...
    nk_flags event5 = nk_edit_string_zero_terminated(
        ctx, NK_EDIT_SIMPLE, buf_Platform_Platform_length, 36, nk_filter_float);
          if (event5 & NK_EDIT_ACTIVATED) { SDL_StartTextInput(); }

          if (event5 & NK_EDIT_DEACTIVATED) { SDL_StopTextInput(); }
...
nk_layout_row_dynamic(ctx, RH / 4, 1);
    ...
    if (event6 & NK_EDIT_ACTIVATED) { SDL_StartTextInput(); }

    if (event6 & NK_EDIT_DEACTIVATED) { SDL_StopTextInput(); }
    ...
    if (event7 & NK_EDIT_ACTIVATED) { SDL_StartTextInput(); }

    if (event7 & NK_EDIT_DEACTIVATED) { SDL_StopTextInput(); }
    ...
    if (event8 & NK_EDIT_ACTIVATED) { SDL_StartTextInput(); }

    if (event8 & NK_EDIT_DEACTIVATED) { SDL_StopTextInput(); }
    ...
    if (event9 & NK_EDIT_ACTIVATED) { SDL_StartTextInput(); }
    if (event9 & NK_EDIT_DEACTIVATED) { SDL_StopTextInput(); }
    ...
    if (event10 & NK_EDIT_ACTIVATED) { SDL_StartTextInput(); }
    if (event10 & NK_EDIT_DEACTIVATED) { SDL_StopTextInput(); }
      ...
      nk_combo_end(ctx);
       ...
       // /silt
            ...
            nk_flags event110 =   nk_edit_string(ctx, NK_EDIT_BOX, box_buffer,  &box_len, 1024 ,nk_filter_float);
                  if (event110 & NK_EDIT_ACTIVATED) { SDL_StartTextInput(); }
                  if (event110 & NK_EDIT_DEACTIVATED) { SDL_StopTextInput(); }
